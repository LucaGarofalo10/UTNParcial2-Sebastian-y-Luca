...
def precentacion_ta_te_ti(tablero):
    os.system("cls")
    print(
        "Parece que te has desviado del camino, pero si le ganas este ta-te-ti a un vagabundo te guiara de vuelta al camino\n"
    )
    mostrar_ta_te_ti(tablero)
    print(
...
def comprobacion_recursiva(tablero, inicio, x, y, anterior, iteracion):
    ...
    if iteracion < 3:
        if (
            iteracion == 0
            or tablero[inicio[0]][inicio[1]] == anterior
            and tablero[inicio[0]][inicio[1]] != " "
        ):
            retorno = comprobacion_recursiva(
                tablero, siguiente, x, y, tablero[inicio[0]][inicio[1]], iteracion + 1
            )  # <--- ACA
        else:
            retorno = False, "0"
    else:
